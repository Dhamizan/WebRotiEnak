...
const submit = async () => {
  try {
    const res = await axios.post('/api/kirim-verifikasi-email', {
      email: email.value,
    })
...
<template>
  <div class="flex items-center justify-center min-h-screen bg-gray-100 p-4">
    <div class="w-full max-w-md bg-white rounded-lg shadow-lg p-8">
      <h2 class="text-2xl font-bold text-center text-gray-800 mb-1">Verifikasi Akun Mu</h2>
      <p class="text-sm text-center text-gray-500 mb-6">
        Masukkan email Anda untuk mendapatkan tautan verifikasi akun
      </p>
